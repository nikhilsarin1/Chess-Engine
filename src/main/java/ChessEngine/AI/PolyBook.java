...
// Data processing of Polyglot binary file was possible due to H.G. Muller and his work
// http://hgm.nubati.net/book_format.html

// Opening Book Titans.bin was prepared by Flavio Martin
...
public class PolyBook {
  ...
  public static long getPolyKey() {
    long key = 0L;

    for (int i = 0; i < 64; i++) {
      if ((bitboard.pieceBitboards[0] >> i & 1) == 1) {
        key ^= polyKeys[64 * 11 + i];
      }
      if ((bitboard.pieceBitboards[1] >> i & 1) == 1) {
        key ^= polyKeys[64 * 9 + i];
      }
      if ((bitboard.pieceBitboards[2] >> i & 1) == 1) {
        key ^= polyKeys[64 * 7 + i];
      }
      if ((bitboard.pieceBitboards[3] >> i & 1) == 1) {
        key ^= polyKeys[64 * 5 + i];
      }
      if ((bitboard.pieceBitboards[4] >> i & 1) == 1) {
        key ^= polyKeys[64 * 3 + i];
      }
      if ((bitboard.pieceBitboards[5] >> i & 1) == 1) {
        key ^= polyKeys[64 + i];
      }
      if ((bitboard.pieceBitboards[6] >> i & 1) == 1) {
        key ^= polyKeys[64 * 10 + i];
      }
      if ((bitboard.pieceBitboards[7] >> i & 1) == 1) {
        key ^= polyKeys[64 * 8 + i];
      }
      if ((bitboard.pieceBitboards[8] >> i & 1) == 1) {
        key ^= polyKeys[64 * 6 + i];
      }
      if ((bitboard.pieceBitboards[9] >> i & 1) == 1) {
        key ^= polyKeys[64 * 4 + i];
      }
      if ((bitboard.pieceBitboards[10] >> i & 1) == 1) {
        key ^= polyKeys[64 * 2 + i];
      }
      if ((bitboard.pieceBitboards[11] >> i & 1) == 1) {
        key ^= polyKeys[i];
      }
    }
  ...
  public static Move getBookMove(long polyKey) {
    List<Move> bookMoveList = new ArrayList<>();
    List<Long> entries = openingBookData.get(polyKey);
    ...
    for (long entry : entries) {
      short move = (short) ((entry >> 48) & 0xFFFF);
      int originFile = (move >> 6) & 7;
      int originRank = (move >> 9) & 7;
      int destinationFile = (move) & 7;
      int destinationRank = (move >> 3) & 7;
      int origin = (7 - originRank) * 8 + originFile;
      int destination = (7 - destinationRank) * 8 + destinationFile;
      int promotionPiece = (move >> 12) & 7;
      ...
      bookMoveList.add(new Move(origin, destination, ' ', ' ', false, false, false, promotion));
    }

    return getRandomMove(bookMoveList);
  }

  private static Move getRandomMove(List<Move> bookMoveList) {
    SecureRandom secureRandom = new SecureRandom();
    int randomIndex = secureRandom.nextInt(bookMoveList.size());
    return bookMoveList.get(randomIndex);
  }

  public Map<Long, List<Long>> readPolyBookFromFile(String filePath) {
    Map<Long, List<Long>> dataMap = new HashMap<>();

    try {
      InputStream inputStream = PolyBook.class.getResourceAsStream(filePath);

      if (inputStream != null) {
        DataInputStream dataInputStream = new DataInputStream(inputStream);

        // Read the data in chunks of 16 bytes (size of each entry)
        byte[] buffer = new byte[ENTRY_SIZE];
        while (dataInputStream.read(buffer) == ENTRY_SIZE) {
          // Extract the first 8 bytes as the key (big-endian format)
          long key = byteArrayToLong(buffer, 0);

          // Extract the next 8 bytes as the value (big-endian format)
          long value = byteArrayToLong(buffer, 8);
